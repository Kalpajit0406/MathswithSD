...
class _SplashScreenState extends State<SplashScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // 1. Setup loading bar progress animation (3 seconds duration)
    ...
    // When progress finishes, orchestrate safe navigation transition
  ...
  void _checkAndNavigate() {
    ...
    // If AuthProvider state has resolved (i.e. is no longer 'initial')
    if (auth.status != AuthStatus.initial) {
      _navigateToNext(auth);
    } else {
      // Otherwise, wait for AuthProvider check to finish
      _authListener = () {
        final updatedAuth = Provider.of<AuthProvider>(context, listen: false);
        if (updatedAuth.status != AuthStatus.initial) {
  ...
  void _navigateToNext(AuthProvider auth) {
    if (!mounted) return;
    final nextWidget = auth.isAuthenticated ? const StudentDashboard() : const WelcomeScreen();

    // Smooth, cinematic 600ms CrossFade page route transition
  ...
  Widget build(BuildContext context) {
    // Responsive aspect ratio & sizing calculations
    final screenSize = MediaQuery.of(context).size;
    final width = screenSize.width;
    final height = screenSize.height;

    // Relative center point of the open book
    ...
    return Scaffold(
      ...
      body: Stack(
        ...
        children: [
          // 1. Vector Scenic Background (Gradient, Sun, Mountains, Trees, Book)
          Positioned.fill(
          ...
          // 2. Faint Watermark "M" in Center Background
          Positioned(
            top: bookCenterY - 145,
            child: IgnorePointer(
              child: Text(
          ...
          // 3. Gentle Floating Math Symbols
          // Symmetrical out-of-phase floating around the open book
          FloatingMathSymbol(
          ...
          // 4. Premium Branding Text (Center Top Area)
          Positioned(
            top: height * 0.22,
            child: Column(
              ...
              children: [
                ...
                // Slogan "PRACTICE TODAY, EXCEL TOMORROW"
                Text(
          ...
          // 5. Loading Bar, Percent Text, and Moving Bike (Bottom Area)
          Positioned(
            bottom: height * 0.08,
            left: 28,
            right: 28,
            child: AnimatedBuilder(
              ...
              builder: (context, child) {
                ...
                return Row(
                  ...
                  children: [
                    // Synchronized Progress Bar & Motorcycle Track
                    Expanded(
                      child: LayoutBuilder(
                        builder: (context, constraints) {
                          final trackWidth = constraints.maxWidth;
                          
                          // Bike settings: width=52, height=32
                          const bikeWidth = 52.0;
                          const bikeHeight = 32.0;
                          
                          // We synchronize the bike's rear wheel contact point (approx 16px from the bike's left edge)
                          // with the current width of the progress fill line.
                          // Rear wheel contact point x_contact = progress * trackWidth
                          // x_bike = progress * trackWidth - 16
                          // We clamp so the bike remains within safe limits.
                          final bikeLeft = (progress * trackWidth - 16.0).clamp(-12.0, trackWidth - bikeWidth + 6.0);
                          
                          // Subtle vertical jitter to simulate bike engine vibration while moving
                          ...
                          return SizedBox(
                            ...
                            child: Stack(
                              ...
                              children: [
                                // Progress track background (Grey)
                                Container(
                                ...
                                // Progress active fill (Orange with glowing border/boxShadow)
                                ...
                                // Dynamic Motion Trail (Speed lines stretching behind the motorcycle)
                                if (isMoving) ...[
                                  // Line 1: Top speed line
                                  Positioned(
                                ...
                                // Himalayan Bike AVIF Image (Synchronized & Animated)
                                Positioned(
                                  left: bikeLeft,
                                  bottom: 4.5 + jitterY, // sits on top of the 4.5px progress bar
                                  child: RepaintBoundary(
                                    child: AvifSafeImage(
                    ...
                    // Percentage text (Fixed size layout preventing shift jitter)
                    Container(
                      width: 42,
                      ...
                      child: Text(
                        ...
                        style: const TextStyle(
                          ...
                          fontFeatures: [
                            FontFeature.tabularFigures(), // Monospaced numbers prevent layout shifts
                          ],
                        ),
                      ),
                    ),
...
// ==========================================
// BACKGROUND VECTOR GRAPHICS CUSTOM PAINTER
// ==========================================
class BackgroundSceneryPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 1. Background Gradient (Soft White to Cream)
    final bgPaint = Paint()
    ...
    // 2. Rising Sun (Soft Radial Orange Glow)
    ...
    // 3. Mountains Layer 1 (Back mountain - Left Peak)
    ...
    // 4. Mountains Layer 2 (Front mountain - Right Peak and Sun overlap)
    ...
    // 5. Stylized Pine Trees (Faint silhouettes on the sides)
    ...
    // Draw page layers/thickness underneath (3 pages layers)
  ...
  void _drawPineTree(Canvas canvas, double x, double y, double width, double height, Paint paint) {
    final path = Path()
      ..moveTo(x, y - height) // Top peak
      ..lineTo(x - width / 2, y) // Bottom left
      ..lineTo(x + width / 2, y) // Bottom right
      ..close();
    canvas.drawPath(path, paint);

    // Add a smaller overlapping layer
...
// ==========================================
// GEOMETRIC TRIANGLE CUSTOM PAINTER
// ==========================================
...
// ==========================================
// INDEPENDENT FLOATING MATH SYMBOL WIDGET
// ==========================================
...
// ==========================================
// PERFORMANCE-OPTIMIZED AVIF RENDERING WIDGET
// ==========================================
class AvifSafeImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AvifImage.asset(
      ...
      height: height,
      fit: fit,
      // Fallback in case of AVIF compilation issues or platform-specific failures
