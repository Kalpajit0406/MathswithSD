...
extension ApiServiceRetry on ApiService {
  ...
  Future<String> startAttemptWithRetry(String examId) async {
    ...
    return resilient.executeWithRetry(
      ...
      operationName: 'Start Attempt',
    );
  }

  /// Execute with retry for submitting answers
  ...
  /// Execute with retry for fetching announcements
  Future<List<Announcement>> getAnnouncementsWithRetry({String? targetClass}) async {
    final resilient = getResilientClient();
    return resilient.executeWithRetry(
      () => getAnnouncements(targetClass: targetClass),
      timeout: const Duration(seconds: 15),
